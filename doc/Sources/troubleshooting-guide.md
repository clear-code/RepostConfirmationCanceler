...
title: |
  ...
  トラブルシューティングガイド
subject: BrowserStartupLauncherトラブルシューティングガイド
date: 2025/09/08
author: 株式会社クリアコード
keywords: [BrowserStartupLauncher, Troubleshooting guide]
titlepage: true
toc-title: 目次
toc-own-page: true
---
...
以下の手順でBrowserStartupLauncherのログを採取できます。
障害発生時には以下の手順でログを採取いただき、サポート窓口までご送付ください。

## EdgeおよびBrowserStartupLauncherのEdge拡張機能のデバッグログの採取手順
...
## BrowserStartupLauncher.exeのログ採取手順

1. `%AppData%\BrowserStartupLauncher`配下の以下のファイルを採取する。
  * `BrowserStartupLauncher_server.log`
  * `BrowserStartupLauncher_server.log.N`（Nは1から10までの整数）
  * `BrowserStartupLauncher_client.log`
  * `BrowserStartupLauncher_client.log.N`（Nは1から10までの整数）
